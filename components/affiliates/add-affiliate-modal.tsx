...
export function AddAffiliateModal({ onClose, onCreated }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/affiliates", {
      ...
      body: JSON.stringify({
        ...
        billing_vat: form.billing_vat || undefined,
      }),
    });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">Add Affiliate</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => set("billing_vat", e.target.value)}
              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[hsl(236,85%,55%)]"
            />
          </div>
